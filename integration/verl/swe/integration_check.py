...
def fetch_gold_fix(offset: int) -> dict[str, str]:
    ...
    with urllib.request.urlopen(R2E_ROWS_URL.format(offset=offset), timeout=60) as r:
...
async def main() -> int:
    ...
    parser.add_argument("--row_offset", type=int, default=0, help="row of the instance in the HF dataset")
    ...
    assert commit.startswith(extra_info["base_commit"][:12]) or extra_info["base_commit"].startswith(
        commit[:12]
    ...
    write_cmds = "\n".join(
        f"cat > {path} << 'SWE_INTEGRATION_EOF'\n{content}SWE_INTEGRATION_EOF" for path, content in src_files.items()
    )
    turns = [
