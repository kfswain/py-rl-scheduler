...
def run(cmd: list[str], timeout: int = 60) -> tuple[int, str]:
    """Returns (rc, stdout) — stderr only when stdout is empty. kubectl exec
    prints 'Defaulted container ...' on stderr for multi-container pods, which
    must not pollute parsed output."""
    try:
...
def pod_exec(pod: str, script: str, timeout: int = 120, namespace: str = "default") -> tuple[int, str]:
...
def check_sandbox_environment(args) -> None:
    ...
    for verb, resource in [("create", "sandboxes.agents.x-k8s.io"), ("get", "pods/exec"), ("get", "pods")]:
        rc, out = kubectl(["auth", "can-i", verb, resource, "-n", args.namespace,
...
def check_ray_cluster(args) -> list[str]:
    ...
    rc, out = pod_exec(heads[0], '[ -n "$WANDB_API_KEY" ] && echo yes || echo no', namespace=args.ray_namespace)
    ...
    rc, out = kubectl(["get", "configmap", "scheduler-config", "-n", args.ray_namespace, "-o", "name"])
...
def check_data(args, pods: list[str]) -> str | None:
    ...
    probe = (
        ...
        "        out[f] = {'rows': len(df), 'keys': sorted(keys), 'image': df.iloc[0]['extra_info']['docker_image']}\n"
    ...
    reports = {}
    for pod in pods:
        rc, out = pod_exec(pod, probe, timeout=180, namespace=args.ray_namespace)
        try:
    ...
    for f in [args.train_file, args.calibrated_file, args.eval_file]:
        ...
        if errors:
            record("FAIL", f"parquet {f}", f"unreadable on {list(errors)}: {list(errors.values())[0]}")
            continue
        rows = {r["rows"] for r in per_pod.values()}
        if len(rows) != 1:
            record("FAIL", f"parquet {f} row counts match", str({p: r['rows'] for p, r in per_pod.items()}))
            continue
        missing_keys = REQUIRED_EXTRA_INFO_KEYS - set(per_pod[head]["keys"])
        if missing_keys:
            record("FAIL", f"parquet {f} schema", f"missing extra_info keys: {sorted(missing_keys)} - rerun preprocessing")
            continue
        record("PASS", f"parquet {f}", f"{rows.pop()} rows on all {len(pods)} pods, schema OK")
        if f == args.calibrated_file:
...
def check_sandbox_smoke(args, image: str) -> None:
    ...
    try:
        ...
        while time.monotonic() - start < args.smoke_timeout:
            rc, phase = kubectl(["get", "pod", SMOKE_SANDBOX, "-n", args.namespace,
        ...
        if phase != "Running":
            record("FAIL", "sandbox smoke: pod Running",
                   f"phase={phase or 'none'} after {args.smoke_timeout}s (capacity? image pull? policy?)")
            return
        elapsed = time.monotonic() - start
        rc, out = kubectl(["exec", "-n", args.namespace, SMOKE_SANDBOX, "--",
                           "sh", "-c", "[ -d /testbed ] && echo TESTBED_OK || echo NO_TESTBED"], timeout=60)
        if "TESTBED_OK" in out:
            record("PASS", "sandbox smoke", f"create->Running {elapsed:.0f}s, exec OK, /testbed present ({image.rsplit('/',1)[-1][:40]})")
        else:
...
def check_hygiene(args, pods: list[str]) -> None:
    ...
    strays = [line.split()[0] for line in out.splitlines()
              if line.strip() and line.split()[0] != SMOKE_SANDBOX] if rc == 0 else []
    record("PASS" if not strays else "WARN", "no stray sandboxes",
           "" if not strays else f"{len(strays)} lingering (capacity leak): {strays[:3]}...")

    for pod in pods[1:]:
        rc, out = pod_exec(pod, "df -BG / | tail -1 | awk '{print $4}'", namespace=args.ray_namespace)
        try:
            free_gb = int(out.strip().rstrip("G"))
            record("PASS" if free_gb > 100 else "WARN", f"disk on {pod}", f"{free_gb}G free")
        except ValueError:
    ...
    for path in ["integration/verl/examples/swe_agent_loop.yaml",
        ...
        record("PASS" if os.path.exists(path) else "FAIL", f"repo file {path}",
               "" if os.path.exists(path) else "run from the repo root?")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--namespace", default="agents-system", help="sandbox namespace (policy-exempt)")
    ...
    parser.add_argument("--skip-smoke", action="store_true", help="skip the live sandbox create/exec test")
    ...
    print(f"\npreflight: {len(RESULTS) - len(fails) - len(warns)} pass, {len(warns)} warn, {len(fails)} fail")
