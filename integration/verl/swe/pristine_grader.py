...
def _run_pytest_ids(client, name: str, ids_json: str, label: str) -> bool:
    """Run a JSON list of pytest ids (newline file + xargs -0: id lists can be
    long and exec commands travel in the request URL)."""
...
def grade_patch_pristine(
    ...
    try:
        ...
        client.write_file(name, "/tmp/agent.patch", filtered)
        ...
        client.write_file(name, "/tmp/test.patch", extra_info["test_patch"])
