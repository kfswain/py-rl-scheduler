...
class SWEAgentLoop(AgentLoopBase):
    ...
    def _init_tunables(self) -> None:
        """Env-tunable knobs, grouped so test drivers that bypass __init__ can
        initialize them in one call (see integration_check.py)."""
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs: Any) -> AgentLoopOutput:
                ...
                if command is None:
                    ...
                    with simple_timer("tool_calls", metrics):
                        ...
                        rc, out = await self.loop.run_in_executor(
                            None, lambda c=command: self._run_command(client, sandbox_name, c)
                        )
        ...
        except Exception as e:  # sandbox/infra failure: salvage the trajectory
