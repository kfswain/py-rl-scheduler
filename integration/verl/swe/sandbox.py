...
def make_name(prefix: str, instance_id: str) -> str:
    ...
    digest = hashlib.sha1(instance_id.encode()).hexdigest()[:4]
...
class SandboxClient:
    ...
    def create(
        ...
        *,
        ...
        self.custom.create_namespaced_custom_object(GROUP, VERSION, self.namespace, PLURAL, manifest)
    ...
    def wait_ready(self, name: str, timeout: float = 300) -> float:
        ...
        while time.monotonic() - start < timeout:
            try:
                ...
                if e.status != 404:  # pod not created yet
                    raise
            time.sleep(3)
    ...
    def exec(
        ...
        for attempt in range(retries + 1):
            ...
            except Exception as e:  # noqa: BLE001 - websocket layer raises bare Exceptions
                last = e
    ...
    def write_file(self, name: str, path: str, content: str | bytes) -> None:
        ...
        chunks = [data[i : i + self._WRITE_CHUNK] for i in range(0, len(data), self._WRITE_CHUNK)] or [b""]
