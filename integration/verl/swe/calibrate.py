...
from integration.verl.swe.sandbox import SandboxError, get_thread_client
...
def run_suite(client: SandboxClient, name: str) -> tuple[dict[str, str], float]:
    ...
    if rc == 124 or elapsed > TIME_CAP_S:
...
def calibrate_instance(namespace: str, extra_info: dict) -> dict:
    ...
    except Exception as e:  # keep the sweep alive; record and move on
...
def main() -> None:
    ...
    parser.add_argument("--parquet", required=True, help="train parquet from prepare_swe_dataset.py")
    parser.add_argument("--out", required=True, help="results JSONL path")
    parser.add_argument("--keep_list", default=None, help="write kept instance_ids here (default: <out>.keep)")
    ...
    with open(args.out, "w") as f, ThreadPoolExecutor(max_workers=args.concurrency) as pool:
    ...
    with open(args.out) as f, open(keep_path, "w") as k:
