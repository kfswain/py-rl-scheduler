...
async def main() -> int:
    ...
    if verl_hook._VERL_LAYOUT != "modern":
        raise RuntimeError("expected modern layout on this verl build")

    servers = {f"srv-{i}": FakeServer.remote(f"srv-{i}") for i in range(N_SERVERS)}
    ...
    for i in range(N_REQUESTS):  # same prefix, growing tail: multi-turn shape
    ...
    ok = (
        n_endpoints == N_SERVERS
        # Prefix-sticky: all same-prefix requests hit one server.
        and max(routed.values()) == N_REQUESTS
