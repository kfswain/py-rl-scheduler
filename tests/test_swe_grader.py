...
def test_spec_key_strips_module_path():
    ...
    assert spec_key("r2e_tests.test_2.TestNested", "test_param[a-1]") == "TestNested.test_param[a-1]"
...
def test_grade_r2e_key_drift_fails():
    ...
    assert not missing.passed and missing.keys_missing == ["T.test_a"]
    extra = grade_r2e({"T.test_a": PASSED, "T.test_new": PASSED}, expected)
    assert not extra.passed and extra.keys_extra == ["T.test_new"]
